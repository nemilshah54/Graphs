		...
		while (!q.empty())
		{
			...
			for (edge &e : vertices[u].outgoing) 
			{
				...
				if (report[v].state == UNDISCOVERED) 
				{
					...
					if (u != src)
					{
						// The edge with least distance is assign to lessD by this loop.
						for (edge &e : vertices[v].incoming)
						{
							if (lessD == -1)
							{
								lessD = report[e.vertex_id].dist;
							}
								
							if (lessD > report[e.vertex_id].dist)
							{
								lessD = report[e.vertex_id].dist;
							}
					...
					// This loops add the npaths of all incoming edjes. (Note only those edges with least distance.)
					for (edge &e : vertices[v].incoming)
					{
						if (report[e.vertex_id].dist == lessD)
						{
							report[v].npaths = report[e.vertex_id].npaths + report[v].npaths;
						}
...
public:
	...
	*/
	...
	void getWay ( int search_in, int fp,  vector< vertex_label> & rp, int source)
	{
		std::queue<int> ww;
		int x;
		int y;
		vector <edge> ee;	
		rp[source].state = DISCOVERED;	
		rp[source].npaths =1;
	...
	while(!ww.empty())
	{
		...
		for ( edge &e : ee)
		{
			...
			if( rp[y].state == UNDISCOVERED)
			{
				rp[y].state= DISCOVERED;
				rp[y].npaths = rp[x].npaths;	
				ww.push(y);
			}
			...
			else
			{
				...
				rp[y].npaths  =    rp[x].npaths  +  rp[y].npaths;
			}
	...
	bool dag_num_paths(vector<vertex_label> & rpt) 
	{
		...
		vector< vertex_label> ip;
		vector< vertex_label> op;
		...
		while( total_nodes > k)
		{
			...
			if( op[k].state== UNDISCOVERED && vertices[k].outgoing.size()==0)
			{
		...
		while (total_nodes > j)
		{
			...
			rpt[j].npaths= ip[j].npaths* op[j].npaths;
			j++;
		}
...
void ts ( int i, bool vv[], stack<int> &stk ) 
{
...
bool sort(std::vector<int> &order)
{
    stack<int> stk;
	...
	*/
	...
	void possible_paths(int start, int destination, vector<string> &p, vector<string> &journey, int vv[])
	{
		journey.push_back(id2name(start));
		...
		else
		{
			for (edge &e : vertices[start].outgoing)
			{
				...
				if (!vv[v])
				{
					...
					possible_paths(v, destination, p, journey, vv);
				}
	...
	void mark(int start, int target, vector<string> &p)
	{
		vector <string> journey;

		int total_nodes = num_nodes();

		int *v = new int[total_nodes];

		int temp = 0;

		while (total_nodes > temp)
		{
		...
		possible_paths(start, target, p, journey, v);
